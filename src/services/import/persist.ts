...
export async function persistImportedChapters(chapters: { title: string; content: any }[], params: PersistChapterParams) {
  ...
  for (const ch of chapters) {
    ...
    if (actNode) {
      ...
      nodes.push(
        ...
        },
        {
          ...
          data: {
            ...
            linkedNodeIds: [],
            isExpanded: true,
            data: { title: `${ch.title || 'Chapter'} - Scene 1`, description: `Opening scene of ${ch.title || 'chapter'}`, goal: '', chapter: id, characters: [], worlds: [], timelineEventIds: [] },
          },
        }
      );
      ...
      if (chIdx !== -1 && sceneNodeId) {
        ...
        if (!cn.data.childIds.includes(sceneNodeId)) cn.data.childIds.push(sceneNodeId);
        nodes[chIdx] = cn;
      }
    }
    ...
    await dalCreateChapter({
      ...
      linkedScenes: sceneNodeId ? [sceneNodeId] : undefined,
  ...
  const contentWithBeat = sceneNodeId ? withSceneBeat(ch.content, sceneNodeId, id, ch.title || 'Chapter') : ch.content;
    const words = countWordsFromTipTapJSON(contentWithBeat);
    if (!contentWithBeat.metadata) contentWithBeat.metadata = { totalCharacters: 0, totalWords: 0, lastEditedAt: new Date().toISOString() };
    contentWithBeat.metadata.totalWords = words;
    ...
    await dalPutChapter({
      ...
      linkedAct: params.linkedAct,
      linkedPlotNodeId: chapterNodeId,
      linkedScenes: sceneNodeId ? [sceneNodeId] : (prevRow as any)?.linkedScenes,
      sortIndex: createdIds.length + 1,
      wordCount: words,
...
export async function startImportDocxJob(input: { file?: File; arrayBuffer?: ArrayBuffer; sceneDivider?: string; bookId: string; versionId: string; userId: string; linkedAct?: string; }) {
  ...
  try {
    ...
    const chapters = await convertDocxToChapters(buf, input.sceneDivider || '***');
