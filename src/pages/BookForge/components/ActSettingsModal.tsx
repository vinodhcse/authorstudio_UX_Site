...
interface ActSettingsModalProps {
  isOpen: boolean;
  onClose: () => void;
  actId: string;
  initialTitle?: string;
  initialDescription?: string;
  onUpdateAct: (actId: string, updates: { title?: string; description?: string }) => Promise<void>;
}

const ActSettingsModal: React.FC<ActSettingsModalProps> = ({ isOpen, onClose, actId, initialTitle = '', initialDescription = '', onUpdateAct }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsSaving(true);
    try {
      await onUpdateAct(actId, { title: title.trim() || undefined, description: description.trim() || undefined });
      onClose();
    } catch (error) {
      ...
      {isOpen && (
        ...
        >
          <motion.div
            ...
            className="bg-gradient-to-br from-gray-200 to-gray-50 dark:from-gray-900 dark:to-black rounded-2xl w-full max-w-lg p-6 shadow-2xl border border-gray-200/50 dark:border-gray-800/50"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-bold text-gray-900 dark:text-white">Act Settings</h2>
              <button
                onClick={handleClose}
                disabled={isSaving}
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label htmlFor="act-title" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Title</label>
                <input
                  id="act-title"
                  type="text"
                  value={title}
              ...
              <div>
                <label htmlFor="act-description" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Description</label>
                <textarea
                  id="act-description"
                  value={description}
...
export default ActSettingsModal;
