...
import { Version, Character, PlotArc, Scene, Chapter, CollaboratorRole } from '../types';
    ...
    const loadBooks = async () => {
      try {
        ...
        for (const bookRow of localBookRows) {
          ...
          const book = bookRow as Book; // Use DAL Book type directly
          bookMap.set(book.id, book);
          // Local books are typically authored by the user
          if (book.authorId === stateUser.id) {
            authoredBookIds.add(book.id);
          } else {
            const me = book.collaborators?.find(c => c.id === stateUser.id);
            if (me) {
              if (me.role === 'EDITOR') {
                editableBookIds.add(book.id);
              } else if (me.role === 'REVIEWER') {
                reviewableBookIds.add(book.id);
              }
            }
